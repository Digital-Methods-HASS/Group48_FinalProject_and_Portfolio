...
Example: Just want to get text from a single page (e.g. Page 31)? 
```{r single-page}
got_p31 <- got_text[31]
got_p31
```

See how that compares to the text in the PDF on Page 31. What has pdftools added and where?
#pdftools has added a lot of newlines (`\n`) to the text, which is how it separates lines in the PDF.
...
#If you convert a word to a number, and it is NOT a number, then it will be NA.
...
#The afinn lexicon assigns a score of 2 to words that are "positive" but not super positive.
...
# The data is a bit messy. Let's make it more nice by using only the top 50 most common "2" words:
got_afinn2_n50 <- got_afinn2_n %>%
  count(word, sort = TRUE) %>%
  head(50) %>%
  mutate(word = fct_reorder(factor(word), n))
  
  ggplot(data = got_afinn2_n50, aes(x = word, y = n)) +
  geom_col() +
  coord_flip() +
  theme_bw()
...
In Game of Thrones the most common meaningful words are words such as "stone", "time", "watch", "lord", "found", "dark", "leave" etc. in their respective categories. I expect that the volume is dominated by negative emotions as the book revolves around a lot of death and conquering. Words such as "stone" = anger, and "watch" = fear are similarly ambiguous to "confidence".
